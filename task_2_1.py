...
def calculate_accuracy(predictions, targets, threshold=0.5):
    """Calculate pixel-wise accuracy for binary masks"""
    pred_binary = (torch.sigmoid(predictions) > threshold).float()
    correct_pixels = (pred_binary == targets).float()
...
def train_epoch(model, dataloader, criterion, optimizer, device):
    ...
    for rgb_sequences, modal_sequences, amodal_sequences, amodal_sequences_full in tqdm(dataloader, desc="Training"):
        ...
        # Calculate metrics (average across all frames in sequence)
        total_loss += loss.item()
        total_accuracy += calculate_accuracy(outputs, amodal_sequences)
        total_iou += calculate_iou(outputs, amodal_sequences)
        num_batches += 1
...
for epoch in range(num_epochs):
    ...
    # Training phase
    train_loss, train_acc, train_iou = train_epoch(model, train_dataloader, criterion, optimizer, device)
    train_losses.append(train_loss)
    train_accuracies.append(train_acc)
    train_ious.append(train_iou)

    print(f"Train - Loss: {train_loss:.4f}, Accuracy: {train_acc:.4f}, IoU: {train_iou:.4f}")
...
final_test_loss, final_test_acc, final_test_iou = test_epoch(model, test_dataloader, criterion, device)
...
print(f"\nPerformance Summary:")
print(f"{'Split':<15} {'Loss':<10} {'Accuracy':<10} {'IoU':<10}")
print("-" * 50)
print(f"{'Final Train':<15} {train_losses[-1]:<10.4f} {train_accuracies[-1]:<10.4f} {train_ious[-1]:<10.4f}")
print(f"{'Final Test':<15} {final_test_loss:<10.4f} {final_test_acc:<10.4f} {final_test_iou:<10.4f}")
...
print(f"  - Ground truth amodal masks shown for ALL frames")
...
def save_multiple_predictions(model, dataloader, device, num_images=20, threshold=0.5, save_dir="test_predictions"):
    ...
    with torch.no_grad():
        for batch_idx, (rgb_sequences, modal_sequences, amodal_sequences, amodal_sequences_full) in enumerate(dataloader):
            ...
            # OPTIMIZATION: Move everything to CPU once to avoid repeated GPU-CPU transfers
            ...
            for i in range(batch_size):
                ...
                # OPTIMIZATION 1: Smaller figure size (reduced from 20, 4*total_frames)
                ...
                for frame_idx in range(total_frames):
                    # Column 0: RGB Image
                    # OPTIMIZATION 2: Direct numpy conversion (faster than ToPILImage)
                    ...
                    axes[frame_idx, 1].set_title(f'Modal {frame_idx+1}', fontsize=10)
                    axes[frame_idx, 1].axis('off')
                    
                    # Column 2: Ground Truth Amodal (now available for all frames)
                    amodal_gt = amodal_vis_full[i, frame_idx].numpy()
                    title = 'GT (Center)' if frame_idx == center_frame_idx else 'GT Amodal'
                    axes[frame_idx, 2].imshow(amodal_gt, cmap='gray')
                    axes[frame_idx, 2].set_title(title, fontsize=10)
                    ...
                    if frame_idx == center_frame_idx:  # Center frame
                        ...
                        axes[frame_idx, 3].set_title('Pred Cont.', fontsize=10)
                        ...
                        axes[frame_idx, 4].set_title(f'Pred Bin.', fontsize=10)
                ...
                # OPTIMIZATION 3: Simpler layout
                fig.suptitle(f'Sample {saved_count+1} (Center: {center_frame_idx+1})', fontsize=12)
                plt.tight_layout()
                
                # OPTIMIZATION 4: Lower DPI and no bbox_inches='tight' (major speedup)
                save_path = os.path.join(save_dir, f'video_sequence_{saved_count:03d}.png')
                plt.savefig(save_path, dpi=75)  # Reduced from 150 DPI
                plt.close(fig)
